...
def neofetch():
    ...
    if "Linux" in system and not distro:
        distro = "Linux"
    
    if system == "Unknown":
        logo = ["Sistema operativo no detectado"]
    else:
        logo = icons.get(distro if distro else system, icons["Unknown OS"])
    ...
    users = psutil.users()
    if users:
        user_info = f"{BOLD}User:{RESET} {users[0].name}"
    else:
        user_info = f"{BOLD}User:{RESET} No users connected"
